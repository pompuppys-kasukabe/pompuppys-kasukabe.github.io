...
function renderProject(){
  ...
  // Itinerary (hidden:true is not shown)
    ...
    if(Array.isArray(menu) && menu.length){
      menuWrap.innerHTML = menu.map(m=>`
        <div class="supportMenuItem">
          ...
          ${m.body ? `<div class="muted" style="line-height:1.75;margin-top:4px;">${escapeHtml(m.body)}</div>` : ""}
        </div>
      `).join("");
  ...
  const sp = document.getElementById("toSponsorPage");
  if(sp){
    sp.href = window.PUPPYS_CONFIG?.pages?.sponsor || "./sponsor.html";
  }

  // Share
  const shareBtn = document.getElementById("projectShareBtn");
  if(shareBtn){
    shareBtn.addEventListener("click", async ()=>{
      const shareData = { title: document.title, text: "POM PUPPYS bright クラファン特設", url: location.href };
      try{
