...
function renderNews(){
  ...
  const itemsRaw = window.PUPPYS_CONFIG?.news || [];
  const items = [...itemsRaw].sort((a,b)=> String(b.date).localeCompare(String(a.date)));
...
/**
 * Hero: video/image auto
 * - index.html に <video id="heroVideo"> が無くても、必要なら生成して差し込みます
 */
function renderHeroMedia(){
  ...
  let v = document.getElementById("heroVideo");

  // legacy fallback (もし昔のDOMが残っている場合)
  ...
  const showImage = ()=>{
    const w = wrap || legacyWrap;
    const imageEl = img || legacyImg;
    if(!w || !imageEl) return;
  ...
  v.play().catch(()=>{ /* 自動再生NGなら poster のまま */ });
}
...
/** project badge link (id が無くても class で拾う) */
function wireProjectBadge(){
  const a =
    document.getElementById("projectBadgeLink") ||
    document.querySelector('a.projectBadge');
